...
import { totalPriceHandle } from "@/redux/reduxReducer/AddCourses/allCoursesSlicer";
import { useEffect } from "react";

export default function PaymentPage() {
  const singleCourse = useSelector((state) => state.courseReducer.singleCourse);
  const dispatch = useDispatch();
  const payment = useSelector((state) => state.courseReducer.payment);
  ...
  useEffect(() => {
    ...
    dispatch(totalPriceHandle(totalPrice));
  }, [fee, discount, dispatch]);
  ...
  return (
    <div className=" w-full h-full flex lg:flex-row sm:flex-col items-center justify-between gap-10 p-10 ">
        ...
        <div className=" flex flex-col gap-4">
          ...
          <p className="text-xl flex items-center justify-between text-indigo-500">
            ...
            <span className="text-gray-400 text-xl">
              ${singleCourse?.courseFee ? singleCourse?.courseFee : "0.00"}
            </span>
          </p>
          <p className="text-xl flex items-center justify-between text-indigo-500">
            Discount
          ...
          <p className="text-xl flex items-center justify-between text-indigo-500">
            Tax
            <span className="text-gray-400 text-xl">
              ${singleCourse?.tax ? singleCourse?.tax : "0.00"}
            </span>
          </p>
          ...
          <p className="flex items-center justify-between">
            ...
            <span className="text-xl font-bold text-blue-800 ">
              ${discount || tax ? fee - discount + tax : fee}
            </span>
          </p>
        </div>
      </div>
      ...
      {payment ? (
        ...
        <div className=" sm:w-[95%] lg:w-[30%] w-[95%]  p-5 mx-auto mt-20 ">
          <CheckOut />
        </div>
      )}
    </div>
